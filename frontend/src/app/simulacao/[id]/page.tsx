...
export default function Simulacao() {
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col font-sans overflow-hidden">
      ...
      <main className="flex-1 min-h-0 overflow-hidden p-6 max-w-6xl mx-auto w-full grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Painel do Caso */}
        <div className="hidden md:flex flex-col bg-white border border-slate-200 rounded-2xl p-6 shadow-sm overflow-y-auto custom-scrollbar [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full hover:[&::-webkit-scrollbar-thumb]:bg-slate-400">
      ...
      <footer className="bg-white border-t border-slate-200 p-4 flex flex-col items-center">
        <div className="w-full max-w-4xl flex items-center gap-2 bg-slate-50 p-2 rounded-2xl border border-slate-200">
          <button
            onClick={toggleMute}
            title="Ligar/Desligar Microfone"
            className={`p-3 shrink-0 rounded-xl transition-all shadow-sm flex items-center justify-center ${isRecording ? "bg-red-500 text-white shadow-red-200 hover:bg-red-600 animate-pulse" : "bg-slate-200 text-slate-600 hover:bg-slate-300"
              }`}
          >
          ...
          <div className="relative flex items-center w-full">
            <input
              ...
              placeholder={isRecording ? "Pode falar, o paciente está ouvindo..." : "Mudo. Digite a conversa com o paciente."}
              className="w-full bg-white border border-slate-200 text-slate-800 pl-4 pr-14 py-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-medium placeholder-slate-400"
            />
            <button
              onClick={handleManualSend}
              disabled={loadingReply || !(finalTextBuffer + " " + interimText).trim()}
              className="absolute right-2 p-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 transition-colors"
            >
              <Send className="w-5 h-5" />
            </button>
          </div>
        </div>
        <p className="text-slate-400 text-xs text-center hidden md:block mt-2">
