...
function ActiveSimuladoContent() {
  ...
  // Carregar destaques do localStorage (opcional, ou manter apenas na sessão de estado)
  useEffect(() => {
    const saved = localStorage.getItem(`highlights_${sessionId}`);
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        // Filtra para garantir que apenas o novo formato seja carregado
        const cleaned: Record<string, { start: number, end: number }[]> = {};
        Object.entries(parsed).forEach(([key, val]) => {
          if (Array.isArray(val) && val.every(item => typeof item === 'object' && 'start' in item)) {
            cleaned[key] = val as { start: number, end: number }[];
          }
        });
  ...
  const saveHighlights = (newHighlights: Record<string, { start: number, end: number }[]>) => {
    setHighlights(newHighlights);
    localStorage.setItem(`highlights_${sessionId}`, JSON.stringify(newHighlights));
  };

  const handleTextSelection = () => {
    if (!isHighlighterActive || !questionTextRef.current) return;
    ...
    // Calcular offsets relativos ao texto completo do container
    const preSelectionRange = range.cloneRange();
    preSelectionRange.selectNodeContents(questionTextRef.current);
    ...
    const currentHighlights = highlights[questionId] || [];

    // Evita duplicados exatos
    if (!currentHighlights.some(h => h.start === start && h.end === end)) {
      const newHighlights = {
        ...highlights,
        [questionId]: [...currentHighlights, { start, end }]
      };
  ...
  const undoLastHighlight = () => {
    ...
    if (currentHighlights.length > 0) {
      const newHighlights = {
        ...highlights,
        [questionId]: currentHighlights.slice(0, -1)
      };
      saveHighlights(newHighlights);
    }
  };
  ...
  const handleSelect = (letter: string) => {
    ...
    // Se clicar na que já está pendente, desmarcar
    if (pendingSelection === letter) {
      setPendingSelection(null);
    } else {
      setPendingSelection(letter);
    }
  };
  ...
  const toggleEliminate = (e: React.MouseEvent, letter: string) => {
    e.stopPropagation(); // Não seleciona a alternativa ao clicar em eliminar
    if (showResults || (mode === "treino" && answers[currentIndex])) return;

    const currentEliminated = eliminated[currentIndex] || [];
    let newEliminated;
    if (currentEliminated.includes(letter)) {
      newEliminated = currentEliminated.filter(l => l !== letter);
    } else {
      newEliminated = [...currentEliminated, letter];
    }

    setEliminated({
      ...eliminated,
      [currentIndex]: newEliminated
    });
  };

  const handleNext = () => {
    setPendingSelection(null); // Resetar seleção pendente
                  ...
                  return (
                    <div key={theme} className="group bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500 space-y-6">
                      <div className="flex items-start justify-between">
                        ...
                        <span className={`text-xs font-black px-3 py-1.5 rounded-xl ${isHigh ? "bg-emerald-50 text-emerald-600" :
                      ...
                      <div className="space-y-3">
                        <div className="w-full bg-slate-100 h-2.5 rounded-full overflow-hidden">
                          <div
                            className={`h-full transition-all duration-1000 ease-out group-hover:opacity-80 ${isHigh ? "bg-emerald-500" : isMid ? "bg-amber-500" : "bg-red-500"
                              }`}
          ...
          <div className="space-y-8">
            ...
            <div className="grid gap-4">
              {questions.map((q, i) => {
                ...
                return (
                  <div key={i} className={`group p-6 rounded-[2rem] border transition-all hover:shadow-md ${(isAnswerCorrect || isAnnulled) ? "bg-white border-slate-100" : "bg-red-50/30 border-red-100/50"
                    }`}>
                    <div className="flex flex-col md:flex-row md:items-center gap-6">
                      <div className={`w-12 h-12 shrink-0 rounded-2xl flex items-center justify-center font-black text-lg ${isAnnulled ? "bg-amber-500 text-white shadow-lg shadow-amber-100" :
                      ...
                      <div className="flex items-center gap-8 shrink-0 md:border-l border-slate-100 md:pl-8">
                        ...
                        <div className="text-center">
                          ...
                          <span className={`w-8 h-8 inline-flex items-center justify-center rounded-lg font-black ${isAnnulled ? "bg-amber-100 text-amber-700" : (isAnswerCorrect ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700")
                            }`}>
                            {answers[i] || "-"}
                          </span>
                        </div>
    ...
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col font-sans">
      ...
      <div className="bg-white/80 backdrop-blur-md border-b border-slate-100 px-6 md:px-12 py-5 flex items-center justify-between top-0 z-20 shadow-sm">
        ...
        <div className="flex items-center gap-8 lg:gap-16">
          <div className={`flex items-center gap-3 px-6 py-3 rounded-[1.25rem] border transition-all duration-500 ${timeLimit === "4h" && remaining < 300
      ...
      <div className="flex-1">
        <div className="max-w-7xl mx-auto p-6 md:p-12 lg:p-20 grid lg:grid-cols-[1fr_380px] gap-12 lg:gap-20">
          ...
          <div className="space-y-12">
            <div className="space-y-8">
              ...
              <div className="relative group">
                ...
                <div
                  ...
                  className={`text-xl md:text-2xl font-bold text-slate-800 leading-[1.6] whitespace-pre-wrap tracking-tight ${isHighlighterActive ? 'cursor-text' : ''}`}
                >
                  {(() => {
                    const text = formatText(currentQ.text);
                    const questionHighlights = highlights[currentQ._id] || [];

                    if (questionHighlights.length === 0) return text;

                    // Ordenar e mesclar intervalos se necessário (ou apenas renderizar um por um)
                    ...
                    sorted.forEach((h, i) => {
                      // Evita sobreposição ou problemas de indexação
                      ...
                      // O destaque
                      result.push(
                        <mark key={`mark-${i}`} className="bg-yellow-200 text-slate-900 rounded-sm shadow-sm border-b border-yellow-300 select-none">
            ...
            <div className="grid gap-5">
              {Object.entries(currentQ.alternatives).map(([letter, text]) => {
                const isConfirmed = answers[currentIndex] === letter;
                const isPending = pendingSelection === letter;
                const isSelected = isConfirmed || isPending;

                const isCorrectChoice = letter === currentQ.correct_answer;
                const isAnnulled = currentQ.correct_answer === "Anulada";
                const isEliminated = eliminated[currentIndex]?.includes(letter);
                ...
                if (isEliminated && !isSelected) {
                  variantClass = "bg-slate-50/50 border-slate-50 text-slate-300 opacity-40 grayscale-[0.5]";
                }
                ...
                if (isSelected) {
                  if (isAnnulled) {
                    ...
                    variantClass = "bg-indigo-50 border-indigo-500 text-indigo-700 shadow-xl -translate-y-1";
                  } else {
                ...
                return (
                  <div key={letter} className="relative group/alt">
                    <button
                      onClick={() => handleSelect(letter)}
                      disabled={showResults || (mode === "treino" && !!answers[currentIndex])}
                      className={`w-full text-left p-6 md:p-8 rounded-[2.25rem] border-2 transition-all duration-300 flex gap-8 items-start relative ${variantClass}`}
                    >
          ...
          <div className="space-y-8 lg:sticky lg:top-[120px] h-fit self-start">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.25em] flex items-center gap-2">
                    <Flag className="w-4 h-4 text-indigo-500" /> Respondidas
                  </h3>
                ...
                {pendingSelection && (
                  <div className="animate-in slide-in-from-bottom-5 duration-500 mb-3">
                    <button
                      ...
                      className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-6 rounded-2xl font-black flex items-center justify-center gap-3 transition-all shadow-2xl shadow-indigo-200 active:scale-95 group"
                    >
                      <Check className="w-6 h-6 group-hover:translate-x-1.5 transition-transform" />
                      Confirmar Alternativa {pendingSelection}
                    </button>
                  </div>
                )}
                <button
            ...
            {showFeedback && (
              <div className="space-y-6 animate-in slide-in-from-right-10 duration-500">
                {/* Main Feedback Card */}
                <div className={`p-10 rounded-[3rem] border-2 shadow-xl ${isCorrect ? "bg-emerald-50 border-emerald-100 shadow-emerald-50" : "bg-red-50 border-red-100 shadow-red-50"
                  }`}>
                  ...
                  <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-[0_20px_50px_rgba(0,0,0,0.04)] space-y-8">
                    ...
                    <div className="space-y-6">
                      {Object.entries(currentQ.explanation.alternatives).map(([key, value]) => {
                        ...
                        return (
                          <div key={key} className={`space-y-2 p-4 rounded-2xl transition-all ${isThisCorrect ? "bg-emerald-50/50 border border-emerald-100" :
                            (isUserChoice ? "bg-red-50/50 border border-red-100" : "bg-slate-50/30")
                            }`}>
                            <div className="flex items-center gap-3">
                              <span className={`w-6 h-6 rounded-lg flex items-center justify-center text-[10px] font-black ${isThisCorrect ? "bg-emerald-500 text-white" :
                                (isUserChoice ? "bg-red-500 text-white" : "bg-slate-200 text-slate-500")
                                }`}>
                                {key}
                              </span>
                              <span className={`text-[10px] font-black uppercase tracking-widest ${isThisCorrect ? "text-emerald-600" :
